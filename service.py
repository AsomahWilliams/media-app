...
outh2_bearer = OAuth2PasswordBearer(tokenUrl="#then u insect ur url link")
...
async def  get_current_user(db: Session, token: str = Depends(outh2_bearer)):
    try:
...
#update user 
async def  update_usser(db: Session, db_user, user_update: UserUpdate):
